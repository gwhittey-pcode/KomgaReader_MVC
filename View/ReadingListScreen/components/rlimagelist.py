...
from kivymd.uix.imagelist.imagelist import SmartTileImage, SmartTileOverlayBox
...
with open(
    os.path.join(os.getcwd(), "View","ReadingListScreen","components", "rlimagelist.kv"), encoding="utf-8"
) as kv_file:
    Builder.load_string(kv_file.read())


class RLSmartTileImage(RectangularRippleBehavior, ButtonBehavior, FitImage, MyAsyncImage):
    """Implements the tile image."""


class RLSmartTileOverlayBox(MDBoxLayout):
    """Implements a container for custom widgets to be added to the tile."""

class RLSmartTile(MDRelativeLayout, ThemableBehavior):
    """
    ...
    def add_widget(self, widget, *args, **kwargs):
        if isinstance(widget, (RLSmartTileImage, RLSmartTileOverlayBox)):
