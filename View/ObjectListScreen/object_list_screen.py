...
class ObjectListScreenView(ComicListsBaseScreenView):
    ...
    def setup_screen(self, object_type="Reading List"):
        ...
        asynckivy.start(self.get_server_lists())
    ...
    async def get_server_lists(self, new_page_num=0):
        ...
        if self.object_type == "Reading List":
            ...
            if len(self.app.filter_publisher_list) != 0:
                add_filter_publisher_list = ""
                for publisher in self.app.filter_publisher_list:
                    s_pb = f"{publisher}"
                    add_filter_publisher_list += f"&publisher={urllib.parse.quote(s_pb)}"
                url_send = f"{url_send}{add_filter_publisher_list}"
