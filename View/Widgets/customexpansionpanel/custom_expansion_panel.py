...
.. code-block:: python
    ...
    self.add_widget(
        CustomeMDExpansionPanel(
            icon="logo.png",  # panel icon
            content=Content(),  # panel content
            panel_cls=CustomeMDExpansionPanelOneLine(text="Secondary text"),  # panel class
        )
    )

To use :class:`~CustomeMDExpansionPanel` you must pass one of the following classes
to the :attr:`~CustomeMDExpansionPanel.panel_cls` parameter:

- :class:`~CustomeMDExpansionPanelOneLine`
- :class:`~CustomeMDExpansionPanelTwoLine`
- :class:`~CustomeMDExpansionPanelThreeLine`
...
.. code-block:: python
    ...
    self.root.ids.box.add_widget(
        CustomeMDExpansionPanel(
            icon="logo.png",
            content=Content(),
            panel_cls=CustomeMDExpansionPanelThreeLine(
                text="Text",
...
.. code-block:: python
    ...
    from kivymd.uix.expansionpanel import CustomeMDExpansionPanel, CustomeMDExpansionPanelThreeLine
...
Two events are available for :class:`~CustomeMDExpansionPanel`
-------------------------------------------------------

- :attr:`~CustomeMDExpansionPanel.on_open`
- :attr:`~CustomeMDExpansionPanel.on_close`
...
__all__ = (
    "CustomeMDExpansionPanel",
    "CustomeMDExpansionPanelOneLine",
    "CustomeMDExpansionPanelTwoLine",
    "CustomeMDExpansionPanelThreeLine",
    "CustomeMDExpansionPanelLabel",
)
...
class CustomeMDExpansionChevronRight(IRightBodyTouch, MDIconButton):
    """Chevron icon on the right panel."""

    _angle = NumericProperty(0)


class CustomeMDExpansionPanelOneLine(OneLineAvatarIconListItem):
    """Single line panel."""


class CustomeMDExpansionPanelTwoLine(TwoLineAvatarIconListItem):
    """Two-line panel."""


class CustomeMDExpansionPanelThreeLine(ThreeLineAvatarIconListItem):
    """Three-line panel."""


class CustomeMDExpansionPanelLabel(TwoLineListItem):
    """
    Label panel.
...
class CustomeMDExpansionPanel(RelativeLayout):
    """
    :Events:
        :attr:`on_open`
    ...
    :class:`~CustomeMDExpansionPanelOneLine`, :class:`~CustomeMDExpansionPanelTwoLine` or
    :class:`~CustomeMDExpansionPanelThreeLine`.
    ...
    def __init__(self, **kwargs):
        ...
        if self.panel_cls and isinstance(
            ...
            if not isinstance(self.panel_cls, CustomeMDExpansionPanelLabel):
                self.chevron = CustomeMDExpansionChevronRight()
            ...
            raise ValueError(
                ...
                "[CustomeMDExpansionPanelOneLine, CustomeMDExpansionPanelTwoLine, "
                "CustomeMDExpansionPanelThreeLine]"
            )
    ...
    def check_open_panel(
        ...
        for panel in self.parent.children:
            if isinstance(panel, CustomeMDExpansionPanel):
    ...
    def set_chevron_up(self, instance_chevron: CustomeMDExpansionChevronRight) -> None:
        """Sets the chevron up."""
    ...
    def _add_content(self, *args):
        if self.content:
            try:
                if isinstance(self.panel_cls, CustomeMDExpansionPanelLabel):
